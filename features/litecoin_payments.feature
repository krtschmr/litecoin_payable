Feature: LitecoinPayment creation, validation and state

Background:
  Given a saved widget
  And a new litecoin_payment
  Then the litecoin_payment field price is set to 10000
    And the litecoin_payment field reason is set to New
  When the litecoin_payment is saved

Scenario: A saved widget can create a payment
Then the litecoin_payment should have an address
  And the litecoin_payment should have the state pending

Scenario: When the payment processor is run the payment status should be pending
When the payment_processor is run
Then the litecoin_payment should have the state pending

Scenario: When a payment is made for 1/2 the amount, the status should be partial payment
When the ltc_amount_due is set
  And a payment is made for 50 percent
When the payment_processor is run
Then the litecoin_payment should have the state partial_payment
...
When the payment_processor is run
Then the litecoin_payment should have the state paid_in_full
